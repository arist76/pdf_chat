    ...
    <div class='bg-gray-100 w-full h-auto p-4 flex flex-col max-w-4xl md:px-8 md:py-1'>
        ...
        <div class="m-4 flex items-center">
            <div class="w-12 h-12 rounded-full flex justify-center items-center overflow-hidden">
                <img class="w-full h-full object-cover" src="{data.imgUrl}" alt="Handsome man with glasses" />
            </div>
            <div class="ml-4">
                <h1 class="font-bold text-xs my-1 sm:text-lg">{data.name}</h1>
                <p class="text-xs">{data.timestamp}</p>
            </div>
        </div>
        
        <!-- user message wrapper -->
        <p class="p-2 text-xs max-w-full sm:text-sm md:ml-2">{data.message}</p> 
        ...
        <div class="text-xs flex items-center p-4 gap-4 sm:text-sm">
            ...
            <div class="flex gap-1">
                <button class="w-4">
                    <svg class="feather feather-thumbs-up stroke-blue-700 hover:stroke-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>
                </button>
                <p><span class="text-blue-800">{data.likes}</span> likes</p>
            </div>
           
            <!-- dislike -->
            <div class="flex gap-1">
                <button class="w-4">
                    <svg class="feather feather-thumbs-down stroke-blue-700 hover:stroke-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>
                </button>
                <p><span class="text-blue-800">{data.dislikes}</span> dislikes</p>
            </div>
        </div>
    </div>
    {/each}
     ...
     <div class="fixed w-full bottom-0 pb-6 z-10 bg-white border-t border-gray-200 pt-2 sm:pt-4 sm:pb-6 dark:bg-slate-900 dark:border-gray-700">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <!-- Input -->
          <div class="relative">
            <textarea class="p-4 pb-12 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600" placeholder="write a message ..."></textarea>
